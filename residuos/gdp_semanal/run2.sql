CREATE DEFINER=root@localhost PROCEDURE Solarity.generar_desglose_perdidas5(
    IN planta_id INT,
...
BEGIN
    ...
    IF agrupar_por_semana THEN
        SELECT 
            DATE_FORMAT(STR_TO_DATE(CONCAT(YEARWEEK(fecha, 1), ' Monday'), '%X%V %W'), '%Y-%m-%d') AS semana_inicio,
